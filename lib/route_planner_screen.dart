...
class _RoutePlannerScreenState extends State<RoutePlannerScreen> {
  final _stops = [RouteStop(), RouteStop()]; // start + end
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
              ...
              child: Column(
                children: [
                  for (int i = 0; i < _stops.length; i++) ...[
                    ...
                    if (i < _stops.length - 1) _StopConnector(canAdd: i == 0 && _stops.length < 5, onAdd: _addStop),
                  ],
          ...
          _BottomBar(resolvedCount: _resolvedCount, total: _stops.length, stops: _stops),
        ],
      ),
    );
  }
...
class _PlaceFieldState extends State<_PlaceField> {
  ...
  void initState() {
    super.initState();
    _focus.addListener(() { if (!_focus.hasFocus) { setState(() => _suggestions = const []); } });
  }
              ...
              GestureDetector(
                onTap: () {
                  ...
                  widget.onPlaceSelected(
                    const PlacePrediction(placeId: '', description: '', mainText: '', secondaryText: ''),
                    null,
                  );
                },
                child: const Padding(
...
class _BottomBar extends StatelessWidget {
  const _BottomBar({required this.resolvedCount, required this.total, required this.stops});
  final int resolvedCount, total;
  final List<RouteStop> stops;

  @override
  Widget build(BuildContext context) {
    final ready = resolvedCount == total;
    return Container(
      padding: const EdgeInsets.fromLTRB(16, 12, 16, 28),
      color: _bgCard,
      child: GestureDetector(
        onTap: ready ? () => _showRouteInfo(context) : null,
        child: Container(
          height: 52,
          decoration: BoxDecoration(
  ...
  void _showRouteInfo(BuildContext context) {
    final coords = stops.map((s) => s.coords!).toList();
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      backgroundColor: _bgCard,
      behavior: SnackBarBehavior.floating,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
      content: Text(
        '${coords.length} stops ready for EV routing',
